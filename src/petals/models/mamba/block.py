"""
Mamba intermediate layer
Based on Hugging Face's transformer implementation.
"""
import math
...
class OptimizedMambaAttention(MambaAttention):
...
class OptimizedMambaDecoderLayer(MambaDecoderLayer):
    def __init__(self, config: MambaConfig):
        nn.Module.__init__(self)
        self.hidden_size = config.hidden_size
        self.self_attn = OptimizedMambaAttention(config=config, layer_idx=0)
        # layer_idx only matters for KV caching, and we re-implement it in Petals
        self.mlp = MambaMLP(config)
        self.input_layernorm = MambaRMSNorm(config.hidden_size, eps=config.rms_norm_eps)
        self.post_attention_layernorm = MambaRMSNorm(config.hidden_size, eps=config.rms_norm_eps)
...
class WrappedMambaBlock(OptimizedMambaDecoderLayer):
    def forward(
        self,
        hidden_states: torch.Tensor,
        *args,
        ...
        **kwargs,
        ...
        if past_key_value is not None:
            ...
            past_key_value = self._reorder_cache_from_bloom_to_mamba(past_key_value, batch_size, past_key_values_length)
        ...
        if use_cache:
            ...
            present_key_value = self._reorder_cache_from_mamba_to_bloom(
    ...
    def _reorder_cache_from_bloom_to_mamba(
    ...
    def _reorder_cache_from_mamba_to_bloom(
        self, key_value: Tuple[torch.Tensor], batch_size: int, seq_length: int
    ) -> Tuple[torch.Tensor]:
        key_states, value_states = key_value
